...
- Identifies the deployment tool (Vercel, Netlify, AWS Amplify, etc.)
...
2. Connect your deployment tool (Vercel, Netlify, etc.)
...
- **Bot login name**: e.g., `vercel[bot]`, `netlify[bot]`
- **Repository homepage URL**: e.g., contains `vercel.app`, `netlify.app`
- **Deployment status URL**: Target URL patterns
- **Repository configuration**: Presence of `.vercel/project.json`, `netlify.toml`, etc.
...
- Multi-channel delivery (Slack, Email, Castrel Chat)

## Tips for Better Results

| Tip | Description |
|-----|-------------|
| **Connect All Tools** | Authorize both GitHub and your deployment platform (Vercel, Netlify, etc.) for complete diagnosis coverage |
...
| **Use Knowledges** | Create [Knowledges](/docs/features/knowledges) to inform Castrel about your project's specific build configurations or known issues |

## Common Questions

::collapsible{name="What deployment tools are supported?"}
Castrel currently supports Vercel, Netlify, AWS Amplify, and GitHub Actions. The extensible architecture allows adding support for additional deployment tools. See [Roadmap](/docs/more/roadmap) for upcoming integrations.
::
...
Yes, Castrel maintains a configurable rule set for determining error severity. You can customize ignorable error patterns through Knowledges to match your team's standards. For example, you can mark certain ESLint warnings or known third-party library warnings as ignorable.
::
